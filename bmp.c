...
char* vigenere_decrypt(const char* key, const char* text){
    ...
    generate_repeating_key(key, repeating_key, text_len, key_len);

    char *decrypted_key = (char *)malloc((text_len + 1) * sizeof(char));
    for(int i = 69; i < text_len + 69; i++){
        char difference = 'C';
        if(text[i-69] >= 65 && text[i-69] <= 90){
            difference = text[i-69] - (repeating_key[i-69]-64);
        } else {
            difference = toupper(text[i-69]) - (repeating_key[i-69]-64);
        }

        if(difference < 65){
            difference += 25;
        }
        decrypted_key[i-69] = difference; //65 po 90
    }
